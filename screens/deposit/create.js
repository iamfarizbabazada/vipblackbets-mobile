...
const PaymentType = ({selected, provider, form, handleSelect, handleCopy}) => {
  ...
  return (
    <TouchableOpacity activeOpacity={.8} onPress={() => handleSelect(provider)} style={{ marginVertical: 10, gap: 10, backgroundColor: selected == provider.name ? theme.colors.primary : theme.colors.accent, borderRadius: 25, paddingVertical: 10, paddingHorizontal: 10}}>
          <View style={{flexDirection: 'row', alignItems: 'center' }}>
            <RadioButton color={'#252525'} uncheckedColor={'#252525'} status={selected == provider.name ? 'checked' : 'unchecked'} />
          <Image source={provider.image} />
          </View>
          {selected == provider.name && (
            <View>
              <AmountInput form={form} />
              <Input mode="outlined" style={{backgroundColor: '#252525'}} label="ID" onChangeText={(text) => form.setFieldValue('withdrawId', text)} />
            </View>
          )}
    </TouchableOpacity>
  )
