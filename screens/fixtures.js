  ...
  useEffect(() => {
    if (selectedLeague?.id) {
      const fetchMatchDetails = async () => {
        try {
          const response = await apiFootbal.get(`/fixtures?league=${selectedLeague.id}&season=2022`);
    ...
    <View>
      <ScrollView contentContainerStyle={styles.container}>
        {matchData.map(({ fixture, league, teams, goals, score }) => (
          <View key={fixture.id} style={{borderBottomColor: theme.colors.primary, marginBottom: 20, borderBottomWidth: 2, padding: 20}}>
            {/* League Info */}
            ...
            <Text style={styles.venue}>{fixture.venue.name}, {fixture.venue.city}</Text>
